...
function AdminPro({title,products}) {
    return (
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'3rem',flexWrap:'wrap',width:'100%'}}>
                {
                    products.map((product)=>{
                       return <Pro product={product} key={product.id}/>
                    })
                }
            </div>
        </div>
...
function Pro({ product }) {
    ...
    return (
        <div style={{ display: "flex", alignItems: 'center', justifyContent: 'center', width: '350px', boxShadow: '0rem 0.1rem 0.9rem rgb(224, 224, 224)', borderRadius: '2rem', padding: '0.5rem 0.5rem 1rem 0.5rem' }}>
            <div>
                <div style={{ backgroundImage: `url(${product.imageUrl})`, backgroundSize: 'cover', width: '100%', height: '250px', borderRadius: '2rem', backgroundRepeat: "no-repeat" }}>
                ...
                <button
                    className="btn btn-warning" type="button" style={{ padding: 6 + "px" }}>UPDATE</button><button className="btn btn-danger" type="button">DELETE</button>
            </div>
        </div>
    )
}
